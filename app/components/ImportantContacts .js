  ...
  return (
    <div className="p-6 space-y-6 ">
      <div className="flex flex-wrap items-center justify-between **mb-4**">
        ...
        <Button onClick={() => setIsAddingContact(true)} className="bg-gradient-to-r from-purple-500 to-indigo-500 hover:from-purple-600 hover:to-indigo-600">
        ...
        <Card className="bg-purple-50 dark:bg-gray-700 dark:text-gray-200 border-purple-200">
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              {editingContact ? <Edit className="w-5 h-5 text-purple-600" /> : <Plus className="w-5 h-5 text-purple-600" />}
          ...
          <CardContent className="space-y-4">
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => (editingContact ? setEditingContact({ ...editingContact, category: e.target.value }) : setNewContact({ ...newContact, category: e.target.value }))}
              />
            </div>
            <div>
              <label className="block text-sm font-medium mb-2">Type</label>
              <select
                className="w-full p-2 border border-gray-300 dark:bg-gray-600 rounded-md"
            ...
            <div>
              ...
              <Input
                type={editingContact?.type === "website" || newContact.type === "website" ? "url" : "tel"}
                ...
                className="dark:bg-gray-600"
                onChange={(e) => (editingContact ? setEditingContact({ ...editingContact, value: e.target.value }) : setNewContact({ ...newContact, value: e.target.value }))}
              />
            </div>
            <div>
              <label className="block text-sm font-medium mb-2">Description (Optional)</label>
              <textarea
                className="w-full p-2 border border-gray-300 dark:bg-gray-600 rounded-md h-20"
                placeholder="Brief description or notes..."
                value={editingContact ? editingContact.description : newContact.description}
                onChange={(e) => (editingContact ? setEditingContact({ ...editingContact, description: e.target.value }) : setNewContact({ ...newContact, description: e.target.value }))}
              />
            </div>
        ...
        <CardContent>
          {contacts.length === 0 ? (
            <div className="text-center py-8 text-gray-500">
              ...
              <Button className="bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white mt-4" onClick={() => setIsAddingContact(true)}>
            ...
            <div className="space-y-3">
              {contacts.map((contact) => (
                <div key={contact.id} className="flex items-center justify-between p-4 border border-gray-200 rounded-lg dark:hover:bg-gray-500 hover:bg-gray-50 transition-colors">
                  <div className="flex items-center gap-4">
                    {contact.type === "phone" ? <Phone className="w-5 h-5 text-blue-600" /> : <Globe className="w-5 h-5 text-green-600" />}
                    <div>
                      ...
                      <p className="text-sm text-gray-600">
                        {contact.type === "phone" ? (
                          ...
                          <a href={contact.value} target="_blank" rel="noopener noreferrer" className="text-green-500 hover:underline">
                            {contact.value}
                          </a>
                        )}
                      </p>
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button size="sm" variant="ghost" onClick={() => handleDeleteContact(contact.id)} className="text-red-600 hover:text-red-700">
