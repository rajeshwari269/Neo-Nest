...
export default function GrowthChart({ defaultMonths = 24 }) {
  ...
  const getWHOHeight = useCallback((ageMonths, gender) => {
    ...
    const months = Object.keys(heightData)
      .map(Number)
      .sort((a, b) => a - b);
  ...
  const getWHOWeight = useCallback((ageMonths, gender) => {
    ...
    // Linear interpolation for months not in the table
    const months = Object.keys(weightData)
      .map(Number)
      .sort((a, b) => a - b);
  ...
  const customTooltip = ({ active, payload, label }) => {
    if (active && payload && payload.length) {
      ...
      if (data) {
        const validation = data.hasActualData ? validateGrowth(data.actualHeight, data.actualWeight, data.whoHeight, data.whoWeight, label, babyGender) : null;
        ...
        return (
          <div className="bg-white p-4 border border-gray-400 rounded-lg shadow-lg text-sm max-w-xs">
            ...
            <div className="space-y-1">
              <p style={{ color: "#FFA500" }}>WHO Height: {data.whoHeight} cm</p>
                ...
                <>
                  ...
                  {validation && (
                    <div className="mt-2 pt-2 border-t">
                      ...
                      <p className={`text-xs ${validation.weightStatus.color}`}>Weight: {validation.weightStatus.percentile}</p>
                    </div>
                  )}
                </>
              )}
  ...
  if (!babyDOB || !babyGender) {
    return (
      <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-sm max-w-5xl mx-auto">
        <div className="text-center py-8">
  ...
  const latestValidation = latestActual ? validateGrowth(latestActual.actualHeight, latestActual.actualWeight, latestActual.whoHeight, latestActual.whoWeight, latestActual.month, babyGender) : null;
  const comprehensiveGuidance = latestValidation ? getComprehensiveGuidance(latestValidation, latestActual.month, babyGender) : null;

  return (
    <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-sm max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-2xl font-bold text-gray-800 dark:text-gray-200 ">WHO Growth Standards Comparison</h2>
          <p className="text-sm text-gray-600 dark:text-gray-300 mt-1">Based on WHO Child Growth Standards 2006 (50th percentile)</p>
          {babyDOB && babyGender && (
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            <label className="text-sm text-gray-600 dark:text-gray-300">Max Age (months):</label>
            <input
              type="number"
              min={6}
              max={60}
              ...
              className="border px-2 py-1 rounded w-20 text-sm dark:bg-gray-600"
            />
          </div>
        </div>
      </div>
      ...
      {latestActual && (
        <div className="mb-6 p-4 dark:bg-gray-800 bg-white rounded-lg border border-blue-200">
          <h3 className="text-lg font-semibold text-blue-800 mb-3">Latest Measurement Summary</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4  text-sm">
            <div className="bg-white dark:bg-gray-700 p-3 rounded border">
              <div className="text-gray-600 dark:text-gray-200">Age</div>
              <div className="text-lg font-semibold text-blue-600">{latestActual.month} months</div>
            </div>
            <div className="bg-white dark:bg-gray-700 p-3 rounded border">
              <div className="text-gray-600 dark:text-gray-200">Height Status</div>
              <div className={`text-lg font-semibold ${latestValidation.heightStatus.color}`}>{latestActual.actualHeight} cm</div>
              <div className="text-xs text-gray-500 dark:text-gray-200">WHO: {latestActual.whoHeight} cm</div>
              <div className={`text-xs ${latestValidation.heightStatus.color}`}>{latestValidation.heightStatus.percentile}</div>
            </div>
            <div className="bg-white dark:bg-gray-700 p-3 rounded border">
              <div className="text-gray-600 dark:text-gray-200">Weight Status</div>
              <div className={`text-lg font-semibold ${latestValidation.weightStatus.color}`}>{latestActual.actualWeight} kg</div>
              <div className="text-xs text-gray-500 dark:text-gray-200">WHO: {latestActual.whoWeight} kg</div>
              <div className={`text-xs ${latestValidation.weightStatus.color}`}>{latestValidation.weightStatus.percentile}</div>
            </div>
            <div className="bg-white dark:bg-gray-700 p-3 rounded border">
              <div className="text-gray-600 dark:text-gray-200">Total Entries</div>
              ...
              {latestValidation.heightStatus.severity === "urgent" ||
                (latestValidation.weightStatus.severity === "urgent" && <div className="text-xs text-red-600 font-medium">⚠️ Attention Needed</div>)}
            </div>
          </div>
        </div>
      )}
      ...
      {comprehensiveGuidance && (
        <div className="mb-6 ">
          <button
            ...
            className={`w-full flex items-center justify-between p-4 dark:bg-gray-800 border rounded-lg hover:shadow-md transition-all ${
              latestValidation.heightStatus.severity === "urgent" || latestValidation.weightStatus.severity === "urgent" ? "bg-white border-red-200" : "bg-white border-purple-200"
            }`}>
            <div className="flex items-center gap-2">
              {(latestValidation.heightStatus.severity === "urgent" || latestValidation.weightStatus.severity === "urgent") && <AlertTriangle className="w-5 h-5 text-red-600" />}
              <Info className="w-5 h-5 text-purple-600" />
              <h3 className="text-lg font-semibold text-purple-800">Comprehensive Growth Analysis & Professional Guidance</h3>
            </div>
            ...
            <div className="mt-4 space-y-4">
              ...
              <div
                className={`p-4 bg-white dark:bg-gray-800  border rounded-lg ${
                  comprehensiveGuidance.height.priority === "emergency" || comprehensiveGuidance.height.priority === "immediate" ? "border-red-300 bg-red-50" : "border-gray-200"
                }`}>
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <h4 className="font-semibold text-gray-800 dark:text-gray-200 mb-2 flex items-center gap-2">
                      Height Analysis
                      {comprehensiveGuidance.height.priority === "emergency" && <span className="bg-red-100 text-red-800 text-xs px-2 py-1 rounded-full font-bold">URGENT</span>}
                    </h4>
                    <p className="text-gray-700 dark:text-gray-300 mb-3">{comprehensiveGuidance.height.message}</p>
                    <div
                      className={`p-3 rounded border-l-4 dark:bg-gray-700 ${
                        comprehensiveGuidance.height.priority === "emergency" || comprehensiveGuidance.height.priority === "immediate" ? "bg-red-50 border-red-400" : "bg-blue-50 border-blue-400"
                      }`}>
                    ...
                    <div className="mt-3 flex items-center gap-2">
                      <a href={comprehensiveGuidance.height.whoLink} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-xs text-blue-600 hover:text-blue-800 underline">
              ...
              <div
                className={`p-4 bg-white dark:bg-gray-800 border rounded-lg ${
                  comprehensiveGuidance.weight.priority === "emergency" || comprehensiveGuidance.weight.priority === "immediate" ? "border-red-300 bg-red-50" : "border-gray-200"
                }`}>
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <h4 className="font-semibold text-gray-800 dark:text-gray-200 mb-2 flex items-center gap-2">
                      Weight Analysis
                      {comprehensiveGuidance.weight.priority === "emergency" && <span className="bg-red-100 text-red-800 text-xs px-2 py-1 rounded-full font-bold">URGENT</span>}
                    </h4>
                    <p className="text-gray-700 dark:text-gray-300 mb-3">{comprehensiveGuidance.weight.message}</p>
                    <div
                      className={`p-3 rounded border-l-4 dark:bg-gray-700 ${
                        comprehensiveGuidance.weight.priority === "emergency" || comprehensiveGuidance.weight.priority === "immediate" ? "bg-red-50 border-red-400" : "bg-green-50 border-green-400"
                      }`}>
                      <h5 className="font-semibold text-sm mb-2">Recommended Actions:</h5>
                      <ul className="text-sm space-y-1">
                    ...
                    <div className="mt-3 flex items-center gap-2">
                      <a href={comprehensiveGuidance.weight.whoLink} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-xs text-blue-600 hover:text-blue-800 underline">
                        <ExternalLink className="w-3 h-3" />
              ...
              <div className="p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                  <div className="bg-white dark:bg-gray-700 p-3 rounded border">
                    <p className="text-gray-600  mb-1 dark:text-gray-200">Height Deviation from WHO Median:</p>
                    <p className={`font-medium ${latestValidation.heightStatus.color}`}>
                      {latestValidation.heightDiff > 0 ? "+" : ""}
                      {latestValidation.heightDiff.toFixed(1)} cm ({latestValidation.heightPercent.toFixed(1)}%)
                    </p>
                    <p className={`text-xs ${latestValidation.heightStatus.color}`}>{latestValidation.heightStatus.percentile}</p>
                  </div>
                  <div className="bg-white dark:bg-gray-700 p-3 rounded border">
                    <p className="text-gray-600 dark:text-gray-200 mb-1">Weight Deviation from WHO Median:</p>
                    <p className={`font-medium ${latestValidation.weightStatus.color}`}>
                      {latestValidation.weightDiff > 0 ? "+" : ""}
                      {latestValidation.weightDiff.toFixed(1)} kg ({latestValidation.weightPercent.toFixed(1)}%)
                    </p>
                    <p className={`text-xs ${latestValidation.weightStatus.color}`}>{latestValidation.weightStatus.percentile}</p>
                  </div>
                </div>
                <div className="mt-3 p-3 bg-blue-50 dark:bg-gray-700 rounded text-xs text-blue-800">
                  <p>
                    <strong>Understanding Percentiles:</strong> The 50th percentile represents the median (average). 25th-75th percentile is considered normal range. Below 5th or above 95th percentile
      ...
      <div className="mb-6">
        <ResponsiveContainer width="100%" height={500}>
          <LineChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 80 }}>
            ...
            <XAxis dataKey="month" label={{ value: "Age (months)", position: "insideBottom", offset: -20 }} height={60} type="number" domain={[0, months]} />
            ...
            {/* WHO Standard Lines */}
            <Line yAxisId="height" type="monotone" dataKey="whoHeight" stroke="#FFA500" strokeWidth={2} name="WHO Height Standard (50th %ile)" dot={false} strokeDasharray="5 5" />
            <Line yAxisId="weight" type="monotone" dataKey="whoWeight" stroke="#20B2AA" strokeWidth={2} name="WHO Weight Standard (50th %ile)" dot={false} strokeDasharray="5 5" />
      ...
      <div className="mt-6 p-4 bg-yellow-50 rounded-lg border border-yellow-200">
        ...
        <p className="text-xs text-yellow-700 mb-3">
          The WHO Child Growth Standards describe how children should grow under optimal conditions. These charts show the 50th percentile (median) values based on data from healthy, breastfed
          children from diverse populations worldwide. Normal growth typically falls within the 3rd to 97th percentiles.
        </p>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-2 text-xs">
          <a href="https://www.who.int/tools/child-growth-standards" target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-yellow-700 hover:text-yellow-900 underline">
            <ExternalLink className="w-3 h-3" />
