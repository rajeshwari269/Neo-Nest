  ...
  return (
    <div className="relative">
      <div
        ...
        className="border px-3 py-2 rounded focus:outline-none focus:ring-2 focus:ring-indigo-500 cursor-pointer flex items-center justify-between bg-white  dark:bg-gray-800">
        <span className={selectedDate ? "text-gray-900 dark:text-gray-200" : "text-gray-500"}>{selectedDate || placeholder}</span>
        <CalendarIcon className="w-4 h-4 text-gray-400" />
      </div>

      {isOpen && (
        <div className="absolute top-full left-0 z-50 mt-1 bg-white dark:bg-gray-800 border rounded-lg shadow-lg p-4 min-w-[280px]">
          <div className="flex items-center justify-between mb-4">
            <button onClick={goToPreviousMonth} className="p-1 hover:bg-gray-100 dark:hover:bg-gray-500 rounded" type="button">
              ←
            </button>
            <div className="font-semibold">
              {monthNames[viewMonth]} {viewYear}
            </div>
            <button onClick={goToNextMonth} className="p-1 hover:bg-gray-100 dark:hover:bg-gray-500 rounded" type="button">
              →
            </button>
          </div>
              ...
              <div key={index} className="text-center">
                {day && (
                  <button
                    ...
                    className={`w-8 h-8 text-sm rounded hover:bg-indigo-100 ${isSelectedMonth && parsedSelectedDate.day === day ? "bg-indigo-500 text-white" : "text-gray-700"}`}
                    type="button">
                    {day}
                  </button>
                )}
          ...
          <div className="mt-4 pt-3 border-t flex justify-between">
            ...
            <button onClick={() => setIsOpen(false)} className="text-sm text-gray-600 hover:text-gray-800 dark:hover:text-gray-500" type="button">
              Cancel
            </button>
          </div>
        </div>
...
export default function GrowthPage() {
  ...
  const getMonthsSinceDOB = (dateStr) => {
    ...
    let months = (entryDate.getFullYear() - birthDate.getFullYear()) * 12 + (entryDate.getMonth() - birthDate.getMonth());
  ...
  const getWHOHeightForBarChart = (dateStr, gender) => {
    ...
    const keys = Object.keys(heightData)
      .map(Number)
      .sort((a, b) => a - b);
  ...
  const saveGrowthLogs = (updatedLogs) => {
    ...
    window.dispatchEvent(
      new CustomEvent("growthDataUpdated", {
  ...
  const addGrowthEntry = () => {
    ...
    let updatedLogs;
    if (editId) {
      updatedLogs = growthLogs.map((log) => (log.id === editId ? { ...log, ...entryWithWHO } : log));
      setEditId(null);
    } else {
      ...
      if (existingDateEntry) {
        ...
        updatedLogs = growthLogs.map((log) => (log.date === newEntry.date ? { ...log, ...entryWithWHO } : log));
      } else {
  ...
  return (
    <div className="container max-w-6xl mx-auto px-4 sm:px-6 space-y-6  dark:bg-gray-800/90">
      <div className="text-center sm:text-left">
        <h2 className="text-2xl sm:text-3xl p-2 font-bold text-gray-800 dark:text-gray-200">Growth Tracker</h2>
        <p className="text-gray-600 dark:text-gray-200 mt-2">Log your baby&apos;s growth, track milestones, and visualize progress over time.</p>
      </div>

      {/* Baby Information Section - WITH PERSISTENT DATA */}
      <div className="bg-white p-4 sm:p-6 rounded-lg shadow dark:bg-gray-700/90">
        <h3 className="text-xl font-semibold mb-4 flex items-center gap-2 dark:text-gray-200">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
          <div>
            ...
            <DatePicker className="dark:bg-gray-800/90" placeholder="Select date of birth" value={babyDOB} onChange={(date) => setBabyDOB(date)} />
          </div>
          <div>
            ...
            <select
              ...
              className="w-full dark:bg-gray-800/90 border px-3 py-2 rounded focus:outline-none focus:ring-2 focus:ring-indigo-500">
        ...
        {babyDOB && (
          <div className="bg-indigo-50 p-3 rounded-lg dark:bg-gray-900/90">
      ...
      {/* Growth Entry Form */}
      <div className="bg-white dark:bg-gray-700/90 p-4 sm:p-6 rounded-lg shadow space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <DatePicker placeholder="Select measurement date" value={newEntry.date} onChange={(date) => setNewEntry({ ...newEntry, date })} />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-2">Height (cm) *</label>
            <Input
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700  dark:text-gray-200 mb-2">Weight (kg) *</label>
            <Input
              className="dark:bg-gray-800/90"
              placeholder="e.g., 7.2"
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700  dark:text-gray-200 mb-2">Head Circumference (cm)</label>
            <Input
              className="dark:bg-gray-800/90"
          ...
          <div className="sm:col-span-2">
            <label className="block text-sm font-medium text-gray-700  dark:text-gray-200 mb-2">Comment</label>
            <Input
              className="dark:bg-gray-800/90"
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <Button className="bg-indigo-500 hover:bg-indigo-600 text-white" onClick={addGrowthEntry} disabled={!babyDOB || !babyGender}>
            <Plus className="w-4 h-4 mr-2" />
            {editId ? "Update Entry" : "Add Entry"}
          </Button>
          {editId && (
            <Button className="bg-gray-500 hover:bg-gray-600 text-white" onClick={cancelEdit}>
              Cancel Edit
            </Button>
          )}
        </div>
      ...
      {growthLogs.length > 0 ? (
        <>
          <div className="bg-white dark:bg-gray-700 p-4 sm:p-6 rounded-lg shadow space-y-4">
            <div className="flex justify-between items-center">
              <h3 className="text-xl font-semibold flex items-center gap-2">
                <BarChart3 className="w-5 h-5 text-blue-500" />
                Growth Log Entries ({growthLogs.length})
              </h3>
              <div className="text-sm text-gray-500 dark:text-gray-400">Most recent first</div>
            </div>
            <div className="space-y-3 max-h-96 overflow-y-auto ">
              {[...growthLogs].reverse().map((log, index) => {
                const months = getMonthsSinceDOB(log.date);
                return (
                  <div key={log.id} className="border rounded-lg p-4 hover:bg-gray-50 dark:hover:bg-gray-600 transition-colors dark:bg-gray-800">
                    <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-3">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-2">
                          <p className="font-medium text-gray-800 dark:text-gray-200">{log.date}</p>
                          <span className="text-xs bg-indigo-100 text-indigo-800 px-2 py-1 rounded">{months.toFixed(1)} months old</span>
                        </div>
                      ...
                      <div className="flex gap-2 self-end sm:self-start">
                        ...
                        <button onClick={() => deleteEntry(log.id)} className="text-red-500 hover:text-red-700 p-1 rounded hover:bg-red-100" title="Delete entry">
          ...
          <div className="bg-white p-4 sm:p-6 rounded-lg shadow overflow-x-auto dark:bg-gray-700">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-4 gap-3">
              ...
              <label className="flex items-center gap-2 text-sm">
                <input type="checkbox" checked={showWHO} onChange={(e) => setShowWHO(e.target.checked)} className="rounded" />
            ...
            <div className="min-w-[600px]">
              <ResponsiveContainer width="100%" height={400}>
                <BarChart
                  ...
                  <XAxis dataKey="shortDate" angle={-45} textAnchor="end" height={80} fontSize={12} />
                  <YAxis yAxisId="left" orientation="left" label={{ value: "Height (cm)", angle: -90, position: "insideLeft" }} fontSize={12} />
                  <YAxis yAxisId="right" orientation="right" label={{ value: "Weight (kg)", angle: 90, position: "insideRight" }} fontSize={12} />
                  <Tooltip
          ...
          {/* Growth Statistics */}
          <div className="bg-white p-4 sm:p-6 rounded-lg shadow dark:bg-gray-700">
            <h3 className="text-xl font-semibold mb-4 flex items-center gap-2">📈 Growth Statistics</h3>
            {(() => {
              if (growthLogs.length < 2) return <p className="text-gray-500">Add more entries to see growth statistics.</p>;
              ...
              return (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="bg-green-50 p-4 rounded-lg dark:bg-gray-800">
                    <h4 className="font-semibold text-green-800 dark:text-green-700 mb-2">Recent Changes</h4>
                    <p className="text-sm text-green-700 dark:text-green-600">
                      Height: {heightGrowth > 0 ? "+" : ""}
                      {heightGrowth} cm
                      <br />
                      Weight: {weightGain > 0 ? "+" : ""}
                      {weightGain} kg
                  ...
                  <div className="bg-blue-50 p-4 rounded-lg dark:bg-gray-800">
                    <h4 className="font-semibold text-blue-800 dark:text-blue-700 mb-2">Total Growth</h4>
                    <p className="text-sm text-blue-700 dark:text-blue-600">
                      Height: +{totalHeightGrowth} cm
                      <br />
                      Weight: +{totalWeightGain} kg
                      <br />
      ...
      {babyDOB && babyGender && (
        <div className="bg-white p-4 sm:p-6 rounded-lg shadow dark:bg-gray-700">
          <h3 className="text-xl font-semibold mb-4 flex items-center gap-2">🌍 WHO Growth Standards Chart</h3>
          <GrowthChart defaultMonths={24} babyData={growthLogs} babyDOB={babyDOB} babyGender={babyGender} />
        </div>
      )}
      ...
      <div className="space-y-6">
        <div className="bg-white dark:bg-gray-700 p-3 sm:p-4 rounded-lg shadow">
          <div className="flex flex-col sm:flex-row items-center justify-center gap-2 sm:gap-4 mb-4 text-center sm:text-left">
            <Baby className="w-6 h-6 sm:w-8 sm:h-8 text-pink-500 dark:text-pink-600" />
            <h1 className="text-xl sm:text-2xl md:text-3xl font-bold bg-gradient-to-r from-pink-600 to-purple-600 bg-clip-text text-transparent">Developmental Tracker</h1>
          </div>
